...
export type Recipient = { id: string; type: 'OWNER' | 'EMPLOYEE' };

export async function getProjectAccessRecipients(tenantId: string): Promise<Recipient[]> {
  ...
  return [
    ...
    ...employees.map((e) => ({ id: e.id, type: 'EMPLOYEE' as const })),
  ];
}
 ...
 */
export async function getProjectSpecificRecipients(projectId: string): Promise<Recipient[]> {
  ...
  return [
    ...(owner ? [{ id: owner.id, type: 'OWNER' as const }] : []),
    ...assignedEmployeeIds.map((id) => ({ id, type: 'EMPLOYEE' as const })),
  ];
}
