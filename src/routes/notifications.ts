...
router.get('/stream', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!decoded.tenantId || !decoded.userId) return res.status(401).json({ error: 'Invalid token' });

    if (decoded.accountType === 'owner') {
      const user = await prisma.user.findUnique({ where: { id: decoded.userId } });
      if (!user || user.tenantId !== decoded.tenantId || !user.isActive) return res.status(401).json({ error: 'Invalid token' });
    } else if (decoded.accountType === 'employee') {
      const employee = await prisma.employee.findUnique({ where: { id: decoded.userId } });
      if (!employee || employee.tenantId !== decoded.tenantId || !employee.isActive) return res.status(401).json({ error: 'Invalid token' });
    }

    addConnection(decoded.userId, decoded.tenantId, req, res);
  } catch (error) {
    if (!res.headersSent) res.status(500).json({ error: 'Failed to establish connection' });
  }
});
...
router.patch('/preferences', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.user?.accountType === 'owner') {
      await prisma.user.update({ where: { id: req.user.id }, data: { notificationPreferences: parsed.data as any } });
    } else {
      await prisma.employee.update({ where: { id: req.user!.id }, data: { notificationPreferences: parsed.data as any } });
    }
