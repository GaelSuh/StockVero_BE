...
export function resolveChannelFlags(
  req: AuthRequest,
  requested: { retailEnabled?: boolean; wholesaleEnabled?: boolean },
): ChannelFlags {
  ...
  if (hasRetail && !hasWholesale) return { retailEnabled: true, wholesaleEnabled: false };
  if (hasWholesale && !hasRetail) return { retailEnabled: false, wholesaleEnabled: true };

  // Neither module active (inventory-only org): leave both on so the product is
  // ready the moment a sales module is switched on.
  if (!hasRetail && !hasWholesale) return { retailEnabled: true, wholesaleEnabled: true };
...
export function channelsAreValid(flags: ChannelFlags): boolean {
  return flags.retailEnabled || flags.wholesaleEnabled;
}

export const CHANNELS_REQUIRED_MESSAGE =
  'A product must be available on at least one sales channel.';
