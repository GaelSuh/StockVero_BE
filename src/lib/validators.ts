...
const LOCAL_LENGTH: Record<string, number> = {
  '+237': 9,   // Cameroon
  '+234': 10,  // Nigeria
  '+233': 9,   // Ghana
  '+254': 9,   // Kenya
...
export function countryToDialCode(country: string): string | null {
...
export function isValidPhone(value: string, country?: string): boolean {
  ...
  const reduced = (s: string) => s.replace(/[^\d]/g, '');
  ...
  if (local.length === localLen) return true;

  // Tolerate a leading 0 that some countries dial locally, e.g. 06XXXXXXXX.
  if (local.length === localLen + 1 && local.startsWith('0')) return true;
...
export function passwordStrengthMessage(password: string): string | null {
  ...
  if (!s.length) return 'Password must be at least 8 characters long.';
  if (!s.symbol) return 'Password must include at least one symbol (e.g. @, #, $, !).';
  return null;
}
