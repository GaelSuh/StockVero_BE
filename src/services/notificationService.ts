...
import { pushToUser, getConnectionsForTenant } from './sseManager.js';
...
function moduleFromType(type: string): string {
  ...
  if (prefix === 'administration') return 'admin';
  if (prefix === 'customer') return 'crm';
  if (prefix === 'contacts') return 'crm';
  if (prefix === 'employee') return 'admin';
  if (prefix === 'settings') return 'system';
  return prefix;
}

/** Fetch the persisted notification preferences for a user or return defaults. */
async function getUserPrefs(userId: string, userType: 'OWNER' | 'EMPLOYEE'): Promise<NotifPrefs> {
  try {
    let raw: unknown;
    if (userType === 'OWNER') {
    ...
    return { ...DEFAULT_PREFS, ...(raw as Partial<NotifPrefs>) };
  } catch {
...
export async function sendNotification(params: SendNotificationParams) {
  try {
    ...
    if (!categoryEnabled) return null; // user has turned off this category
...
export async function broadcastToTenant(tenantId: string, params: BroadcastParams) {
  try {
 ...
 */
export async function broadcastToModule(tenantId: string, moduleKey: string, params: BroadcastParams) {
  try {
