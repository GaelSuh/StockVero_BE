...
export async function runProjectDueDateCheck(): Promise<void> {
  const now = new Date();
  ...
  for (const project of projects) {
    ...
    try {
      const daysOverdue = Math.floor(
        (todayStart.getTime() - project.dueDate!.getTime()) / (1000 * 60 * 60 * 24),
      );
      ...
      console.log(
        `[ProjectDueDates] Project '${project.name}' (${project.tenantId}): ${
  ...
  for (const phase of phases) {
    ...
    try {
      const daysOverdue = Math.floor(
        (todayStart.getTime() - phase.endDate!.getTime()) / (1000 * 60 * 60 * 24),
      );
      ...
      totalNotified += recipients.length;
      console.log(
        `[ProjectDueDates] Phase '${phase.name}' in '${phase.project.name}': ${
          daysOverdue === 0 ? 'due today' : `${daysOverdue}d overdue`
        } — notified ${recipients.length} users.`,
      );
    } catch (err) {
      console.error(
        `[ProjectDueDates] Failed to process phase '${phase.name}' in '${phase.project.name}':`,
        err,
      );
    }
  }
  ...
  // ── Summary ───────────────────────────────────────────────────────────────
  console.log(
    `[ProjectDueDates] Job complete. Projects checked: ${projectCount}. Phases checked: ${phaseCount}. Total notifications sent: ${totalNotified}.`,
  );

  // Collect unique tenantIds from processed projects and phases to write one audit entry per tenant
  const tenantIds = [
    ...new Set([
      ...projects.map((p: any) => p.tenantId),
    ]),
  ];
