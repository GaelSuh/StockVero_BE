 ...
 */
function actorTypeFromUser(user: { accountType: string }): AuditActorType {
 ...
 */
async function assertVariantQuantityWithinCeiling(
  tx: any,
  tenantId: string,
  category: { id: string; quantityOnHand: number; stockTrackingMode: string },
  requestedQuantity: number,
  excludeVariantId?: string,
): Promise<void> {
  if (category.stockTrackingMode !== 'QUANTITY') return;
  ...
  const allocated: number = others._sum.quantityOnHand ?? 0;

  if (allocated + requestedQuantity > category.quantityOnHand) {
    const room = Math.max(category.quantityOnHand - allocated, 0);
    throw new VariantQuantityCeilingError(
      `Only ${room} unit(s) can be allocated to variants right now — ${allocated} of the ` +
 ...
 */
function labelFrom(attributes: Record<string, string>, axisOrder: string[]): string {
  return axisOrder
    .map((k) => attributes[k])
 ...
 */
async function buildVariantLabel(
  tx: any,
  tenantId: string,
  categoryId: string,
  attributes: Record<string, string>,
  excludeVariantId?: string,
): Promise<{ label: string; axisOrder: string[] }> {
  ...
  const referenceOrder: string[] =
    existing?.axisOrder?.length ? existing.axisOrder : Object.keys(attributes);
 ...
 */
function uniqueViolationResponse(error: unknown, res: Response): boolean {
...
export const createVariant = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!category) return res.status(404).json({ success: false, message: 'Product not found' });
    ...
    if (uniqueViolationResponse(error, res)) return;
 ...
 */
export const generateVariants = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const user = req.user!;
    const { categoryId } = req.params;
    const { attributes, quantityOnHand } = req.body as {
      attributes?: Array<{ name?: string; values?: string[] }>;
      /** Starting stock applied to EVERY variant this batch creates. */
    ...
    for (const raw of attributes) {
      ...
      if (axes.some((a) => a.name.toLowerCase() === name.toLowerCase())) continue;
    ...
    if (total > MAX_GENERATED_COMBINATIONS) {
      return res.status(400).json({
        success: false,
        message:
          `That would create ${total} variants, past the limit of ${MAX_GENERATED_COMBINATIONS}. ` +
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      const taken = new Set(
        existing.map((v: any) => combinationKey((v.attributes ?? {}) as Record<string, string>)),
      );
      ...
      const startOrder = existing.reduce((max: number, v: any) => Math.max(max, v.sortOrder ?? 0), 0);
    ...
    await logAudit({
      ...
      details: {
        ...
        labels: result.toCreate.map((c: Record<string, string>) => labelFrom(c, axisOrder)),
      },
    });
 ...
 */
export const assignUnitsToVariant = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const user = req.user!;
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      const eligibleIds = eligible.map((u: any) => u.id);
    ...
    if (result.assigned === 0) {
      return res.status(422).json({
        success: false,
        message:
          'None of those units could be assigned. They must belong to this product and still be in stock.',
      });
    }
...
export const listVariants = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const { categoryId } = req.params;
...
export const listAllVariantsForTenant = async (req: AuthRequest, res: Response) => {
  try {
    const variants = await prisma.productVariant.findMany({
...
export const setVariantActive = (isActive: boolean) => async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const user = req.user!;
    const existing = await prisma.productVariant.findFirst({ where: { id: req.params.id, tenantId } });
 ...
 */
export const canDeleteVariant = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const variant = await prisma.productVariant.findFirst({ where: { id: req.params.id, tenantId } });
    if (!variant) return res.status(404).json({ success: false, message: 'Variant not found' });
...
export const deleteVariant = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (itemCount > 0) blockers.push(`${itemCount} stock unit${itemCount === 1 ? '' : 's'}`);
    if (priceRuleCount > 0) blockers.push(`${priceRuleCount} price rule${priceRuleCount === 1 ? '' : 's'}`);
    if (saleItemCount > 0) blockers.push(`${saleItemCount} sale record${saleItemCount === 1 ? '' : 's'}`);
    if (returnItemCount > 0) blockers.push(`${returnItemCount} return record${returnItemCount === 1 ? '' : 's'}`);
    if (stockLogCount > 0) blockers.push(`${stockLogCount} stock log entr${stockLogCount === 1 ? 'y' : 'ies'}`);

    if (blockers.length > 0) {
      return res.status(409).json({
        success: false,
        message: `Cannot delete "${variant.label}" — it is still referenced by ${blockers.join(
          ', ',
        )}. Deactivate it instead so it stays out of new sales while its history stays intact.`,
      });
    }
