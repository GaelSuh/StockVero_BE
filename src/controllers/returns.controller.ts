 ...
 */
export async function getReturnsAnalytics(req: AuthRequest, res: Response) {
  try {
    const tenantId = req.tenantId!;
    ...
    if (req.query.mode) where.sale = { mode: req.query.mode };
    ...
    const byProduct = new Map<string, { productName: string; quantity: number; value: number }>();
    for (const r of returns) {
