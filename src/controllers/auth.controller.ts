...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    const tenantModules = Array.from(
      new Set([...selectedModules, MODULE_KEYS.BILLING, MODULE_KEYS.SETTINGS, MODULE_KEYS.ADMINISTRATION]),
    );
...
function getDefaultTheme(): Record<string, any> {
  return {
    primary: '#0EA5E9',
    secondary: '#0F172A',
    background: '#F0F9FF',
...
function respondWithTenantStatus(
  res: Response,
  status: string,
...
export const verifyOtp = async (req: Request, res: Response) => {
  try {
    ...
    void logAudit({
      ...
      actorId: record.id,
      actorName: `${(record as any).firstName} ${(record as any).lastName}`,
      action: 'OTP_VERIFIED',
      module: 'auth',
      ...extractRequestContext(req),
    });
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    void logAudit({
      tenantId: (decoded as any).tenantId ?? 'unknown',
      actorType: userType === 'OWNER' ? AuditActorType.OWNER : AuditActorType.EMPLOYEE,
      actorId: userId,
