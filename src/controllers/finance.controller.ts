...
export const listTransactions = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const invoiceIds = (rawTxs as any[]).filter((tx) => (tx as any).invoiceId).map((tx) => (tx as any).invoiceId as string);
    ...
    const transactions = (rawTxs as any[]).map((tx) => ({ ...tx, invoiceNumber: (tx as any).invoiceId ? (invoiceMap[(tx as any).invoiceId] ?? null) : null }));
...
export const getTransaction = async (req: AuthRequest, res: Response) => {
  try {
    const tx: any = await (prisma as any).transaction.findFirst({
...
export const updateTransaction = async (req: AuthRequest, res: Response) => {
  try {
    ...
    void logAudit({
      ...
      entityId: transaction.id,
      entityLabel: transaction.description ?? undefined,
      details: buildDiff(existing as any, transaction as any),
      ...extractRequestContext(req),
    });
...
export const getSummary = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const where: any = { tenantId: req.tenantId!, status: 'ACCEPTED' };
    if (projectId) {
      where.OR = [
        { projectId },
    ...
    if (from || to) { where.recordedAt = {}; if (from) where.recordedAt.gte = from; if (to) where.recordedAt.lte = to; }
    ...
    const [income, expense, pendingInvoiceCount, pendingInvoiceValue, approvedThisMonth, rejectedThisMonth, balanceRecord, pendingExpenseValue] = await Promise.all([
      ...
      (prisma as any).invoice.count({ where: { tenantId: req.tenantId!, status: 'PENDING' } }),
      (prisma as any).invoice.aggregate({ where: { tenantId: req.tenantId!, status: 'PENDING' }, _sum: { total: true } }),
      (prisma as any).invoice.count({ where: { tenantId: req.tenantId!, status: 'APPROVED', updatedAt: { gte: startOfMonth, lte: endOfMonth } } }),
      (prisma as any).invoice.count({ where: { tenantId: req.tenantId!, status: 'REJECTED', updatedAt: { gte: startOfMonth, lte: endOfMonth } } }),
      (prisma as any).tenantFinanceBalance.findUnique({ where: { tenantId: req.tenantId! } }),
      (prisma as any).invoice.aggregate({ where: { tenantId: req.tenantId!, status: 'PENDING', type: 'PURCHASE' }, _sum: { total: true } }),
    ]);
    ...
    const summaryApprovedPurchases = await (prisma as any).invoice.findMany({
      where: { tenantId: req.tenantId!, type: 'PURCHASE', status: 'APPROVED' },
      include: { category: { select: { costPrice: true } } },
    });
    const summaryRemainingApprovedCost = (summaryApprovedPurchases as any[]).reduce((sum: number, inv: any) => {
    ...
    return res.json({
      ...
      data: {
        totalRevenue,
        totalExpenses,
        net: (totalRevenue as any) - (totalExpenses as any),
        netBalance,
        totalIncome,
        totalExpense,
...
export const getMonthlySummary = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (Number.isNaN(year)) return res.status(400).json({ success: false, message: 'Invalid year' });
    ...
    for (const tx of transactions) {
      ...
      if (tx.type === 'INCOME') months[monthIndex].income += Number(tx.amount);
      else if (tx.type === 'EXPENSE') months[monthIndex].expense += Number(tx.amount);
    }
...
export const getWeeklySummary = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (Number.isNaN(year) || Number.isNaN(month)) return res.status(400).json({ success: false, message: 'Invalid year or month' });
    ...
    for (const tx of transactions) {
      ...
      if (tx.type === 'INCOME') weeks[weekIndex].income += Number(tx.amount);
      else if (tx.type === 'EXPENSE') weeks[weekIndex].expense += Number(tx.amount);
    }
...
export const getBalance = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const balance = await (prisma as any).tenantFinanceBalance.findUnique({
      where: { tenantId },
    });
    ...
    const remainingApprovedCost = (approvedPurchaseInvoices as any[]).reduce((sum: number, inv: any) => {
      const unitsRemaining = Math.max(0, (inv.authorisedQty ?? 0) - (inv.addedQty ?? 0));
      const costPerUnit = Number(inv.category?.costPrice ?? 0);
      return sum + unitsRemaining * costPerUnit;
    }, 0);
...
export const getAvailableTransactionsForLinking = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
