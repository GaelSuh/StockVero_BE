...
import {
  buildSubscriptionModulePayload,
  calculateProrationAmount,
  ensureTenantModuleEnabled,
  disableTenantModule,
  recalcSubscriptionAmounts,
  toDecimal,
  getPeriodEnd,
...
function detectCardBrand(cardNumber: string | undefined) {
...
function maskPaymentMethod(method: any) {
...
export const getBillingModules = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const tenant = await prisma.tenant.findUnique({
      where: { id: tenantId },
    ...
    const activeModules = tenant.subscriptionModules.filter(
      (m) => m.status !== 'REMOVED',
    );
    const activeKeys = new Set(activeModules.map((m) => m.moduleKey));

    const availableModules = PRICING_MODULES.filter((m) => !activeKeys.has(m.key)).map(
      (m) => ({
        moduleKey: m.key,
...
export const addBillingModule = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
...
export const removeBillingModule = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
...
export const cancelModuleRemoval = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
    const parsed = RemoveModuleSchema.safeParse(req.body);
    if (!parsed.success) {
...
export const getPaymentMethod = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
...
export const requestBillingCycleChange = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
...
export const getBillingTransactions = async (req: AuthRequest, res: Response) => {
  try {
    const tenantId = req.tenantId!;
