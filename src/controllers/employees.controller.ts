...
const serializeEmployee = async <T extends { avatarUrl?: string | null }>(employee: T) => ({
  ...employee,
  avatarUrl: await resolveSignedUrl(employee.avatarUrl),
});

function generateTemporaryPassword(firstName: string, tenantSlug: string) {
...
export const updateEmployee = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (payload.jobTitle !== undefined) updates.jobTitle = payload.jobTitle.trim() || null;
    if (payload.roleId !== undefined) updates.roleId = payload.roleId;
...
export const updateEmployeeStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updates: any = { isActive: parsed.data.isActive };
