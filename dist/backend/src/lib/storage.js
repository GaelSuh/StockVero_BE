 ...
 */
export const PRIVATE_STORAGE_BUCKET = process.env.SUPABASE_PRIVATE_BUCKET || 'stockvero-private';
 ...
 */
export const uploadPrivateFile = async (path, body, contentType) => {
    if (!supabase)
 ...
 */
export const signPrivateFile = async (path, expiresInSeconds = 10 * 60) => {
    if (!supabase)
        return null;
...
export const deletePrivateFile = async (path) => {
    if (!supabase)
        return;
    const { error } = await supabase.storage.from(PRIVATE_STORAGE_BUCKET).remove([path]);
    if (error)
        console.warn('[storage] Failed to delete private file:', error.message);
};
/**
 ...
 */
export const extractStorageKey = (url) => {
    if (!supabaseUrl)
        return null;
    try {
...
export const deleteStorageFiles = async (urls) => {
    if (!supabase || !urls.length)
        return;
    const keys = urls.map(extractStorageKey).filter((k) => Boolean(k));
    if (!keys.length)
        return;
    const { error } = await supabase.storage.from(STORAGE_BUCKET).remove(keys);
    if (error)
        console.warn('[storage] Failed to delete files:', error.message);
};
/**
 ...
 */
export const resolveSignedUrl = async (url) => {
    if (!url)
        return null;
    if (!supabase)
        return url;
