...
export const MODULE_PRICES = Object.fromEntries(PRICING_MODULES.map((m) => [m.key, m.monthlyPrice]));
 ...
 */
export function calcMonthlyTotal(moduleKeys, orgSizeTier) {
    const moduleTotal = moduleKeys.reduce((sum, key) => sum + (MODULE_PRICES[key] ?? 0), 0);
    const surcharge = ORGANISATION_SIZE_SURCHARGES[orgSizeTier]?.monthlySurcharge ?? 0;
 ...
 */
export function calcAnnualTotal(monthly) {
 ...
 */
export function calcProratedAmount(monthlyPrice, daysRemaining, daysInPeriod) {
