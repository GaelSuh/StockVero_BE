...
async function verifySourceRecord(tenantId, sourceModule, sourceId) {
    try {
        let record = null;
        if (sourceModule === 'PROJECT') {
            ...
            record = await prisma.inventoryCategory.findUnique({ where: { id: sourceId }, select: { tenantId: true } });
        }
        else if (sourceModule === 'PRODUCT_ITEM') {
            record = await prisma.productItem.findUnique({ where: { id: sourceId }, select: { tenantId: true } });
        }
...
router.post('/upload', upload.single('file'), async (req, res) => {
    ...
    const { sourceModule, sourceId, docType, description } = req.body;
    if (!VALID_MODULES.includes(sourceModule)) {
    ...
    const source = await verifySourceRecord(tenantId, sourceModule, sourceId);
    if (!source.found) {
...
router.get('/', async (req, res) => {
    const tenantId = req.tenantId;
    const { sourceModule, sourceId } = req.query;
    ...
    const docs = await prisma.document.findMany({
        where: { tenantId, sourceModule: sourceModule, sourceId },
