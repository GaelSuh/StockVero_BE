...
import { listInvoices, getInvoiceSummary, getInvoiceById, approveInvoice, rejectInvoice, createClientInvoiceHandler, sendClientInvoice, getInvoicePdfData, approveProjectInstalment, deleteInvoice, } from '../controllers/invoice.controller.js';
const router = Router();
...
// Can-delete check
router.get('/:id/can-delete', permissionGuard('finance', 'canDelete'), async (req, res) => {
    try {
...
// Delete invoice
router.delete('/:id', permissionGuard('finance', 'canDelete'), async (req, res) => {
    const report = await checkInvoiceDependencies(req.params.id, req.tenantId);
    if (!report.canDelete) {
