...
import { createTransaction, listTransactionsController as listTransactionsCtrl, getTransaction, updateTransaction, deleteTransaction, getSummary, getMonthlySummary, getWeeklySummary, getBalance, getAvailableTransactionsForLinking, } from '../controllers/finance.controller.js';
const router = Router();
 ...
 */
router.get('/transactions/:id/can-delete', permissionGuard('finance', 'canDelete'), async (req, res) => {
    try {
...
router.delete('/transactions/:id', permissionGuard('finance', 'canDelete'), async (req, res) => {
    const report = await checkTransactionDependencies(req.params.id, req.tenantId);
    if (!report.canDelete) {
