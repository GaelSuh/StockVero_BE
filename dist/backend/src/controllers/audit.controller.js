...
export const listAuditLogs = async (req, res) => {
    try {
        ...
        const limit = req.query.limit ? Math.min(100, Math.max(1, parseInt(req.query.limit, 10))) : 25;
...
export const getAuditSummary = async (req, res) => {
    try {
        ...
        const [totalEntries, todayEntries, failureCount, topActorsRaw, topModulesRaw] = await Promise.all([
            ...
            prisma.auditLog.count({ where: { tenantId, status: 'FAILURE' } }),
        ...
        const topActors = topActorsRaw.map((r) => ({ actorName: r.actorName, actorId: r.actorId, count: r._count.id }));
        const topModules = topModulesRaw.map((r) => ({ module: r.module, count: r._count.id }));
        return res.json({
            success: true,
...
export const getAuditActions = async (req, res) => {
    try {
        ...
        return res.json({ success: true, data: actions.map((a) => a.action) });
    }
    catch (error) {
