        ...
        const [customerCount, activeProjectsCount, lowStockItems, recentProjects, summary, inventoryValue, bestSellingProducts,] = await Promise.all([
            ...
            inventoryEnabled
                ? (async () => {
                    ...
                    const stockCounts = await prisma.productItem.groupBy({
                        by: ['categoryId'],
                        where: { tenantId, categoryId: { in: categoryIds }, stockStatus: 'AVAILABLE' },
                        _count: { id: true },
                    });
                    // Single groupBy for INVENTORY AVAILABLE counts
                    const invCounts = await prisma.productItem.groupBy({
                        by: ['categoryId'],
                        where: { tenantId, categoryId: { in: categoryIds }, inventoryStatus: 'AVAILABLE' },
                        _count: { id: true },
                    });
                    const countMap = {};
                    for (const g of stockCounts)
                        countMap[g.categoryId] = g._count.id;
                    for (const g of invCounts)
            ...
            inventoryEnabled
                ? (async () => {
                    ...
                    const countMap = {};
                    for (const g of stockAvail)
                        countMap[g.categoryId] = g._count.id;
                    for (const g of invAvail)
                        countMap[g.categoryId] = g._count.id;
                    let total = 0;
            ...
            inventoryEnabled
                ? (async () => {
                    ...
                    const groups = await prisma.productItem.groupBy({
                        by: ['categoryId'],
                        where: {
                            tenantId,
