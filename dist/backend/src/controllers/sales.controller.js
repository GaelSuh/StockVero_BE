...
import { deductStockForSale, InsufficientStockError } from '../services/saleInventoryService.js';
 ...
 */
const SALE_PAYMENT_METHODS = ['CASH', 'MTN_MOMO', 'ORANGE_MONEY', 'CARD', 'BANK_TRANSFER', 'OTHER'];
...
export async function createSale(req, res) {
    try {
        ...
        const user = req.user;
        const { mode, customerId, creditContact, customerName, items, payments, discountType, discountValue, notes, creditDueDate, } = req.body;
        if (!items?.length) {
        ...
        const badMethod = (payments ?? []).find((p) => String(p?.method).toUpperCase() !== CREDIT_MARKER &&
            !SALE_PAYMENT_METHODS.includes(String(p?.method).toUpperCase()));
        if (badMethod) {
            return res.status(400).json({
                success: false,
        ...
        const offlineId = req.body.offlineId || undefined;
        if (offlineId) {
        ...
        const sellerType = user.accountType === 'owner' ? 'OWNER' : 'EMPLOYEE';
        ...
        const sale = await prisma.$transaction(async (tx) => {
                ...
                if (offlineId) {
                    ...
                    if (Number.isFinite(clientPrice) && clientPrice >= 0) {
                        ...
                        if (deviation <= OFFLINE_PRICE_TOLERANCE) {
                            ...
                            console.warn(`[offline-sale] Price deviation beyond tolerance — tenant ${tenantId}, offlineId ${offlineId}, ` +
            ...
            const requestedUnitIds = items.map((item) => item.unitId || null);
            ...
            const settledPayments = (payments ?? []).filter((p) => String(p.method).toUpperCase() !== CREDIT_MARKER);
            ...
            const rawTotalPaid = settledPayments.reduce((sum, p) => sum + p.amount, 0);
            ...
            const resolvedCustomerName = customerName || creditCustomerName || (effectiveCustomerId ? undefined : 'Walk-in Customer');
            ...
            const { soldUnitIds } = await deductStockForSale(tx, saleItems.map((i, idx) => ({
...
export async function getSale(req, res) {
    try {
        ...
        const soldByName = await backfillPersonName(sale.soldByName, sale.soldById, sale.soldByType);
        return res.json({
            success: true,
            data: {
                ...sale,
 ...
 */
function lookupSaleByCode(mode) {
    return async (req, res) => {
        try {
            ...
            if (!sale)
                return res.status(404).json({ success: false, message: 'No sale found for this code' });
...
export async function updateSale(req, res) {
    try {
        ...
        const { customerName, customerId, notes, discountType, discountValue, items, } = req.body;
        const updateData = {};
        ...
        if (items?.length) {
            ...
            if (updateData.amountOwed <= 0)
                updateData.paymentStatus = 'PAID';
            else if (Number(existing.amountPaid) > 0)
                updateData.paymentStatus = 'PARTIAL';
            else
                updateData.paymentStatus = 'CREDIT';
        }
        if (discountType !== undefined)
            updateData.discountType = discountType || null;
        if (discountValue !== undefined)
            updateData.discountValue = discountValue || null;
...
export async function addPaymentToSale(req, res) {
    try {
        const tenantId = req.tenantId;
        const user = req.user;
        ...
        const newTipAmount = Number(sale.tipAmount ?? 0) + tipAmount;
        await prisma.$transaction(async (tx) => {
            ...
            await recordSaleTipAsIncome(tx, sale, tipAmount, tenantId);
        });
        return res.json({
            success: true,
...
export async function getDailySalesSummary(req, res) {
    try {
        ...
        const mode = queryMode === 'RETAIL' || queryMode === 'WHOLESALE'
            ? queryMode
...
export async function getCreditCustomersController(req, res) {
    try {
...
export async function resolvePricesController(req, res) {
    try {
        const tenantId = req.tenantId;
...
export async function cloneSaleController(req, res) {
    try {
        const tenantId = req.tenantId;
        const saleId = req.params.id;
        const sale = await prisma.sale.findFirst({
