...
const resolveEventType = (before, after) => {
    ...
    if (after === 'DEPLOYED')
        return { eventType: 'DEPLOYED', delta };
    if (after === 'IN_USE')
        return { eventType: 'DEPLOYED', delta }; // INVENTORY type analogue
    if (after === 'UNDER_MAINTENANCE')
        return { eventType: 'MAINTENANCE', delta };
    if (after === 'FAULTY' || after === 'COMPLETELY_BAD')
        return { eventType: 'FAULTY', delta };
    if (after === STATUS_AVAILABLE && before !== STATUS_AVAILABLE)
        return { eventType: 'RETURNED', delta };
...
export const recordStockEvent = async (opts) => {
    try {
        ...
        const countWhere = { tenantId: opts.tenantId, categoryId: opts.categoryId };
        if (isInventoryType) {
        ...
        const currentAvailable = await prisma.productItem.count({ where: countWhere });
        const stockAfter = currentAvailable;
        const stockBefore = stockAfter - opts.delta;
        await prisma.categoryStockLog.create({
            data: {
...
const DEFAULT_IDENTIFIER_LABELS = {
...
export const resolveIdentifierLabel = (identifierType, providedLabel) => {
 ...
 */
const getItemStatus = (item, categoryType) => {
    if (categoryType === 'INVENTORY')
        return item.inventoryStatus ?? null;
 ...
 */
const statusField = (categoryType) => categoryType === 'INVENTORY' ? 'inventoryStatus' : 'stockStatus';
const buildCategoryStats = async (tenantId, categoryId, categoryType) => {
    const field = statusField(categoryType);
    const isStock = categoryType === 'STOCK';
    const [totalItems, availableCount, secondaryCount, maintenanceCount, faultyCount] = await Promise.all([
        prisma.productItem.count({ where: { tenantId, categoryId } }),
        prisma.productItem.count({ where: { tenantId, categoryId, [field]: 'AVAILABLE' } }),
        isStock
            ? prisma.productItem.count({ where: { tenantId, categoryId, stockStatus: 'DEPLOYED' } })
            : prisma.productItem.count({ where: { tenantId, categoryId, inventoryStatus: 'IN_USE' } }),
        prisma.productItem.count({ where: { tenantId, categoryId, [field]: 'UNDER_MAINTENANCE' } }),
...
const buildBatchCategoryStats = async (tenantId, categories) => {
    ...
    const [totalByCategory, stockAvailable, invAvailable, stockDeployed, invInUse, stockMaintenance, invMaintenance, stockFaulty, invFaulty,] = await Promise.all([
        ...
        prisma.productItem.groupBy({ by: ['categoryId'], where: { tenantId, categoryId: { in: categoryIds }, stockStatus: 'DEPLOYED' }, _count: { id: true } }),
        prisma.productItem.groupBy({ by: ['categoryId'], where: { tenantId, categoryId: { in: categoryIds }, inventoryStatus: 'IN_USE' }, _count: { id: true } }),
        prisma.productItem.groupBy({ by: ['categoryId'], where: { tenantId, categoryId: { in: categoryIds }, stockStatus: 'UNDER_MAINTENANCE' }, _count: { id: true } }),
    ...
    try {
        ...
        const category = await prisma.inventoryCategory.create({
            data: {
                ...
                sellingPrice: data.sellingPrice !== undefined ? data.sellingPrice : null,
                plannedQty: data.plannedQty ?? 0,
                plannedDate: data.plannedDate ? new Date(data.plannedDate) : null,
                reorderThreshold: data.reorderThreshold ?? 0,
                identifierType: (data.identifierType ?? 'NONE'),
                identifierLabel,
        ...
        }).catch((err) => console.error('[notify] category created:', err));
        const emptyStats = {
            totalItems: 0,
            availableCount: 0,
    ...
    try {
        ...
        const updated = await prisma.inventoryCategory.update({
            ...
            data: {
                ...
                identifierType: data.identifierType !== undefined ? data.identifierType : undefined,
        ...
        if (data.images !== undefined) {
            ...
            if (removed.length)
                deleteStorageFiles(removed).catch(() => { });
        }
        ...
        broadcastToModule(tenantId, 'inventory', {
            ...
            link: '/inventory/categories',
        }).catch((err) => console.error('[notify] category updated:', err));
        return res.status(200).json({
            success: true,
...
export const deleteCategory = async (req, res) => {
    try {
        ...
        const productItemCount = await prisma.productItem.count({
        ...
        const storedImages = Array.isArray(category.images) ? category.images : [];
        if (category.imageUrl)
            storedImages.push(category.imageUrl);
        const uniqueImages = [...new Set(storedImages)];
        if (uniqueImages.length)
            deleteStorageFiles(uniqueImages).catch(() => { });
        broadcastToModule(tenantId, 'inventory', {
            type: 'inventory.category.deleted',
            title: 'Inventory Category Deleted',
            message: `Category "${category.name}" has been removed.`,
            link: '/inventory/categories',
        }).catch((err) => console.error('[notify] category deleted:', err));
...
export const checkCategoryAvailability = async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const requested = req.query.quantity ? parseInt(req.query.quantity, 10) : 0;
...
export const createProductItem = async (req, res) => {
    try {
        ...
        const resolvedUserIdentifier = identifierMode === 'auto' ? systemId : (userIdentifier?.trim() ?? null);
        ...
        }).catch((err) => console.error('[notify] item created:', err));
        // Record stock event
...
export const restockRequest = async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const submittedBy = req.user?.id;
        if (!submittedBy) {
...
export const listProductItems = async (req, res) => {
    try {
        const tenantId = req.tenantId;
...
export const updateProductItem = async (req, res) => {
    try {
        ...
        if (statusChanged) {
            await prisma.productItemMaintenanceLog.create({
                data: {
                    tenantId,
...
export const deleteProductItem = async (req, res) => {
    try {
        ...
        const catType = item.category?.type ?? 'STOCK';
        ...
        await prisma.productItemMaintenanceLog.deleteMany({ where: { productItemId: req.params.id } });
...
export const logMaintenance = async (req, res) => {
    try {
        ...
        const statusUpdate = catType === 'INVENTORY'
...
export const getMaintenanceLogs = async (req, res) => {
    try {
        ...
        const logs = await prisma.productItemMaintenanceLog.findMany({
