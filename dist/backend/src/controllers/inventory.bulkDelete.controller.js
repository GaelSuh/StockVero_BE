 ...
 */
export const bulkCheckCategories = async (req, res) => {
    try {
        ...
        const reports = await checkInventoryCategoriesDependencies(categories.map((c) => c.id), tenantId);
        const deletable = [];
        const blocked = [];
        ...
        if (deletable.length > 0) {
            deletedIds = await prisma.$transaction(async (tx) => {
                ...
                const nowBlocked = new Set([
                    ...soldRows.map((r) => r.categoryId),
                    ...unitRows.map((r) => r.categoryId),
                    ...priceRows.map((r) => r.categoryId),
                ]);
            ...
            const images = deletable
                ...
                .filter(Boolean);
            if (images.length)
                deleteStorageFiles([...new Set(images)]).catch(() => { });
            ...
            if (deletedIds.length > 0) {
                ...
                }).catch((err) => console.error('[notify] bulk delete:', err));
            }
        }
        return res.json({
            success: true,
