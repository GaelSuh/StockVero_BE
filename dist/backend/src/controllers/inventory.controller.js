...
function resolveStatus(quantity, lowStockAt) {
    if (quantity <= 0)
        return 'OUT_OF_STOCK';
...
export const createMovement = async (req, res) => {
    try {
        ...
        if (data.direction === 'OUT') {
            checkAndNotifyLowStock(req.tenantId, item.id, item.name, newQuantity, item.lowStockAt).catch(() => { });
        }
        return res.status(201).json({
            success: true,
...
export async function checkAndNotifyLowStock(tenantId, productId, productName, currentStock, lowStockAt) {
    try {
        if (currentStock > lowStockAt)
            return;
