 ...
 */
export const normalizeCategoryName = (value) => value.trim().replace(/\s+/g, ' ').toLowerCase();
/** Tidies a display name without changing the author's casing. */
const cleanDisplayName = (value) => value.trim().replace(/\s+/g, ' ');
// ── Schemas ────────────────────────────────────────────────────────────────────
...
const ImportRowSchema = z.object({
     ...
     */
    channels: z.preprocess((v) => (typeof v === 'string' ? v.trim().toLowerCase() : v), z.enum(['retail', 'wholesale', 'both']).nullable().optional()),
});
 ...
 */
const buildAbbreviation = (name, taken) => {
 ...
 */
export const listProductNames = async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const products = await prisma.inventoryCategory.findMany({
            where: { tenantId },
...
export const bulkImportProducts = async (req, res) => {
    try {
        ...
        const channelsFor = (row) => {
            const choice = (row.channels ?? batchChannels);
        ...
        const existingProducts = await prisma.inventoryCategory.findMany({
            where: { tenantId },
            select: { name: true },
        });
        const existingNames = new Set(existingProducts.map((p) => normalizeName(String(p.name ?? ''))));
        ...
        if (barcodeList.length > 0) {
            const clashes = await prisma.inventoryCategory.findMany({
        ...
        // ── Resolve categories ────────────────────────────────────────────────────
        const referencedIds = [...new Set(rows.map((r) => r.categoryId).filter(Boolean))];
        const existingCategories = await prisma.productCategory.findMany({
            where: { tenantId },
        });
        const byId = new Map(existingCategories.map((c) => [c.id, c]));
        const byNormalized = new Map(existingCategories.map((c) => [c.normalizedName, c]));
        ...
        const categoriesToCreate = new Map(); // normalized -> display name
        for (const row of rows) {
            if (row.categoryId || !row.newCategoryName)
                continue;
            const normalized = normalizeCategoryName(row.newCategoryName);
            if (byNormalized.has(normalized) || categoriesToCreate.has(normalized))
                continue;
        ...
        const existingSkus = await prisma.inventoryCategory.findMany({
            where: { tenantId },
        ...
        for (const c of existingSkus) {
            if (c.sku)
                takenSkus.add(String(c.sku).toUpperCase());
            if (c.abbreviation)
                takenSkus.add(String(c.abbreviation).toUpperCase());
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            const resolved = new Map(); // normalized -> category id
            for (const [normalized, category] of byNormalized) {
                resolved.set(normalized, category.id);
            }
            ...
            for (let i = 0; i < rows.length; i += 1) {
                ...
                const product = await tx.inventoryCategory.create({
                    data: {
                        tenantId,
                        name: row.name,
                        sku: abbreviation,
        ...
        broadcastToModule(tenantId, 'inventory', {
            ...
            link: '/inventory',
        }).catch((err) => console.error('[notify] bulk import:', err));
        void logAudit({
            tenantId,
        ...
        if (error instanceof InsufficientFundsError) {
            return res.status(400).json({
                success: false,
                message: 'Could not record what this stock cost. Nothing was saved — choose "I already have it" if this stock was not just bought.',
            });
        }
