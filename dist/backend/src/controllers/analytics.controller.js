...
const resolveDateRange = (year, from, to) => {
...
export const getAnalyticsOverview = async (req, res) => {
    try {
        ...
        const topProducts = materials
            .map((m) => ({
            name: m.name,
            sourceType: (m.sourceType ?? 'EXTERNAL'),
        ...
        const customerRevenueIds = customerRevenueGroups.map((group) => group.entityId).filter((id) => Boolean(id));
        ...
        for (const sale of salesRows) {
            ...
            bucket.total += amount;
            bucket.orders += 1;
            if (sale.mode === 'WHOLESALE')
                bucket.wholesale += amount;
            else
        ...
        const salesAnalytics = {
            ...
            itemsSold: salesRows.reduce((sum, s) => sum + s.items.reduce((n, i) => n + i.quantity, 0), 0),
...
export const exportAnalyticsPdf = async (req, res) => {
    try {
        ...
        const whereRange = (field) => ({
            [field]: { gte: start, lt: end },
        });
        const [customers, projects, inventory, transactions,] = await Promise.all([
        ...
        if (allowedModules.includes('crm')) {
            ...
            if (customers.length === 0)
                doc.text('No customers in this range.');
            doc.moveDown();
        }
        if (allowedModules.includes('projects')) {
            ...
            if (projects.length === 0)
                doc.text('No projects in this range.');
            doc.moveDown();
        }
        if (allowedModules.includes('inventory')) {
            ...
            if (inventory.length === 0)
                doc.text('No inventory items in this range.');
            doc.moveDown();
        }
