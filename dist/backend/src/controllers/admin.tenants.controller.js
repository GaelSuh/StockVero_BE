...
import { PRICING_MODULES, PRICING_MODULE_KEYS, calculatePricing, resolveOrganisationSizeKey, TRIAL_DAYS, } from '../lib/pricing.js';
...
const allowedModuleKeys = new Set(PRICING_MODULE_KEYS);
export const listTenants = async (req, res) => {
    try {
        ...
        const data = tenants.map(tenant => ({
            ...
            enabledModuleCount: tenant.modules.filter(m => m.moduleKey !== MODULE_KEYS.SETTINGS && m.moduleKey !== MODULE_KEYS.BILLING).length,
        }));
...
export const getTenant = async (req, res) => {
    try {
        ...
        const [employeeCount, totalProjects, totalCustomers, totalInventoryItems, totalTransactions, totalIncome, auditLogs, paymentHistory] = await Promise.all([
...
export const updateTenant = async (req, res) => {
    try {
        ...
        for (const field of [
            ...
            if (updates[field] !== undefined && tenant[field] !== updates[field]) {
                const nextValue = typeof updates[field] === 'string' ? updates[field]?.trim() : updates[field];
                data[field] = nextValue;
                changes.push({ field, before: tenant[field], after: nextValue });
            }
        }
        if (changes.length === 0) {
            return res.status(200).json({
...
export const approveTenant = async (req, res) => {
    try {
        ...
        const billingCycleForApproval = (subscription?.billingCycle ?? tenant.billingCycle ?? 'MONTHLY');
...
export const denyTenant = async (req, res) => {
    try {
...
export const getTenantModules = async (req, res) => {
    try {
        ...
        const modules = MODULES_CONFIG.filter(m => m.key !== MODULE_KEYS.SETTINGS && m.key !== MODULE_KEYS.BILLING).map(m => ({
            moduleKey: m.key,
...
export const updateTenantModules = async (req, res) => {
    try {
...
export const updateTenantOwner = async (req, res) => {
    try {
        ...
        for (const field of ['firstName', 'lastName', 'phone']) {
            if (parsed.data[field] !== undefined) {
                const nextValue = typeof parsed.data[field] === 'string'
                    ? parsed.data[field]?.trim()
                    : parsed.data[field];
                if (owner[field] !== nextValue) {
                    updates[field] = nextValue;
                    changes.push({ field, before: owner[field], after: nextValue });
                }
            }
        }
 ...
 */
export const getTenantVerificationDocument = async (req, res) => {
    try {
        const tenant = await prisma.tenant.findUnique({
            where: { id: req.params.id },
            select: {
